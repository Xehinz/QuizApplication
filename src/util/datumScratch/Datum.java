...
public class Datum implements Comparable<Datum> {
	...
	public Datum() throws InvalidAttributesException {
		this(Calendar.getInstance().get(Calendar.DATE), Calendar.getInstance().get(Calendar.MONTH)  ,  Calendar.getInstance().get(Calendar.YEAR)+2);
	}
	...
	public int verschilInMaanden(Datum d) {
		if (d.maand <= 2 && this.maand >= 2 && verschilInDagen(d) >= 12) {
			return (int) Math.floor(((this.convertDatumNaarDagen(this) - d
					.convertDatumNaarDagen(d)) / 30.39));
		} else {
			return (int) Math.floor(((this.convertDatumNaarDagen(this) - d
					.convertDatumNaarDagen(d)) / 30.39) + 0.03);
		}
	}
